...
pub struct TransformGizmoSettings {
    ...
    /// Current gizmo mode.
    pub mode: GizmoMode,
    /// Whether to enable Z-axis components (for 3D mode).
    pub enable_z_axis: bool,
    /// Whether to enable rotation gizmos.
    pub enable_rotation_gizmos: bool,
    /// Whether to enable scale gizmos.
    pub enable_scale_gizmos: bool,
    /// Whether to enable plane translation gizmos.
    pub enable_plane_gizmos: bool,
}
...
impl Default for TransformGizmoSettings {
    fn default() -> Self {
        Self {
            ...
            enable_z_axis: true,
            enable_rotation_gizmos: true,
            enable_scale_gizmos: true,
            enable_plane_gizmos: true,
        }
    }
}
...
impl TransformGizmoPlugin {
    ...
    fn check_dependencies(&self, app: &App) {
        // Check for SelectionPlugin (provides EditorSelection)
        if !app.is_plugin_added::<crate::selection::SelectionPlugin>() {
            panic!(
                "TransformGizmoPlugin requires SelectionPlugin to be registered.\n\
                 This plugin provides the EditorSelection resource used by transform gizmo systems.\n\
                 Please add SelectionPlugin to your app before adding TransformGizmoPlugin:\n\
                 app.add_plugins(SelectionPlugin);"
            );
        }

        info!("TransformGizmoPlugin: All plugin dependencies verified");
    }
}
...
impl Plugin for TransformGizmoPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.insert_resource(TransformGizmoSettings {
            // Configure for 2D mode - disable Z-axis and 3D-specific components
            enable_z_axis: false,         // Disable Z-axis for 2D
            enable_rotation_gizmos: true, // Keep rotation enabled for 2D rotation
            enable_scale_gizmos: true,    // Keep scale enabled for 2D scaling
            enable_plane_gizmos: false,   // Disable plane gizmos (not needed for 2D)
        ...
        app.add_systems(
            ...
            (
                (
                    ...
                    place_gizmo.after(TransformSystem::TransformPropagate),
                )
...
fn drag_gizmo(
    ...
    grid_settings: Query<&InfiniteGridSettings>,
        ...
        match kind {
            InteractionKind::TranslateAxis { original: _, axis } => {
                ...
                if settings.snap_enabled {
                    if let Ok(grid_settings) = grid_settings.single() {
                        translation = snap_to_grid(translation, grid_settings.scale);
                    }
                }
            ...
            InteractionKind::TranslatePlane { normal, .. } => {
                ...
                if settings.snap_enabled {
                    if let Ok(grid_settings) = grid_settings.single() {
                        translation_delta = snap_to_grid(translation_delta, grid_settings.scale);
                    }
                }
...
fn adjust_view_translate_gizmo(
    ...
    // Find the first camera, or fallback to any available camera
    let cam_transform = camera.iter().next();

    if let Ok((mut global_transform, mut interaction)) = gizmo.single_mut()
        && let Some(cam_transform) = cam_transform
    {
...
fn gizmo_cam_copy_settings(
    ...
    // Find the first active main camera, or fallback to the first camera
    let main_camera_data = main_cam
        .iter()
        .find(|(camera, _, _)| camera.is_active)
        .or_else(|| main_cam.iter().next());

    if let Some((main_cam, main_cam_pos, main_proj)) = main_camera_data
