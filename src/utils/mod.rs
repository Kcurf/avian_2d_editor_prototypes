...
fn on_press(trigger: Trigger<Pointer<Pressed>>, mut state: ResMut<DragCancelClickState>) {
    state.0.insert(trigger.target(), Instant::now());
}

fn on_drag_start(trigger: Trigger<Pointer<DragStart>>, mut state: ResMut<DragCancelClickState>) {
